  ...
  <section class="additional-risk">
    <div v-for="(item, index) in allRiskList" :key="index">
      ...
      <div v-show="item.isSelect">
        ...
        <!--配置选项------爱安享B-->
        <div v-if="item.productCode == 'ATRR03'" class="item-tips">
...
export default {
  name: "additionalRisk",
  components: { RiskSettingSelect, FormRadio, FormAmount },
  props: {
    //传进来的险种信息 附带附加险
    selectMain: {
      type: Object,
  ...
  computed: {
    ...mapState({
      ...
      isShowRisk: state => state.plan.isShowRisk //是否执行附加险组件create方法
    })
  },
  watch: {
    mainSettingList: {
    ...
    "selectMain.productCode": {
      //监听主险变化  附加险列表重新初始化  同时清空vuex里面附加险信息
      handler(newval, oldval) {
        console.log(newval, oldval);
  ...
  methods: {
    ...
    initInsurantSocial() {
      this.allRiskList.forEach(item => {
        if (item.productCode == "ATRR03") {
          ...
          insurant.shebao =
            typeof this.insurant.isSocialInsurance == "undefined"
              ? this.insurant.shebao
              : this.insurant.isSocialInsurance;
          this.$store.commit("plan/setState", {
            attr: "insurant",
            data: insurant
          });
    ...
    dealPayPeriod(val, payPeriod) {
      ...
      //几年期
      if (this.isContains(payPeriod, "term_")) {
        let numl = parseInt(payPeriod.replace("term_", ""));
      ...
      } else if (this.isContains(payPeriod, "single")) {
        //暂时处理一次交清
        val.attValue = "single";
        val.attText = "一次交清";
        val.attValueList = productTerm.filter(item => {
    ...
    dealHuoMian(data) {
      ...
      data.attList.forEach(val => {
        ...
        if (val.attCode == "payMode") {
          this.selectMain.attList.forEach(key => {
            if (key.attCode == "payMode") {
              val.attValueList =
                key.attValueList &&
                key.attValueList.filter(item => {
                  let c = [];
                  c.push(this.mainSettingList.payMode);
                  return c.includes(item.code);
                });
          ...
          this.selectMain.attList.forEach(key => {
            if (key.attCode == "payPeriod") {
              ...
              if (
                ...
                //回显数据处理
                console.log(this.riskProposalProductList);
                if (this.riskProposalProductList.length > 0) {
                  this.riskProposalProductList.forEach(item3 => {
                    if (
                      item3.productCode == "ATCR03" ||
                      item3.productCode == "ATCR04"
                    ) {
                      if (data.productCode == item3.productCode) {
                        this.dealPayPeriod(val, item3.payPeriod);
                      }
                    } else if (
                      (data.productCode == "ATCR03" ||
                        data.productCode == "ATCR04") &&
                      ...
                      this.dealPayPeriod(val, this.mainSettingList.payPeriod);
                    }
                  });
                } else {
                  this.dealPayPeriod(val, this.mainSettingList.payPeriod);
          ...
          this.selectMain.attList.forEach(key => {
            if (key.attCode == "policyPeriod") {
              val.attValueList =
                ...
                key.attValueList.filter(item => {
                  let c = [];
                  c.push(this.mainSettingList.policyPeriod);
                  console.log(this.mainSettingList);
                  return c.includes(item.code);
                });
