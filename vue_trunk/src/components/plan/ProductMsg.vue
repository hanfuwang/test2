    ...
    <div
      ...
      v-if="
        totalAmount &&
          mainProposalProductList[0] &&
          sortSetting(mainProposalProductList[0])[0].attCode == 'premium' &&
          mainProposalProductList[0].productCode != 'DTCB02' &&
          mainProposalProductList[0].productCode != 'DTRB03'
      "
    >
      首期总保额：
      <span>{{ totalAmount | global_addCommas }}</span>
      元
    </div>
    <!-- 宝宝驾到特殊处理 显示： 首期总保额：详见条款（详见附件一） -->
    <div
      class="all-pay"
      v-if="
        totalAmount &&
    ...
    <div
      ...
      v-if="
        totalAmount &&
          mainProposalProductList[0] &&
          sortSetting(mainProposalProductList[0])[0].attCode == 'premium' &&
          mainProposalProductList[0].productCode == 'DTCB03'
      "
    >
      首期总保额：详见条款
    </div>
