    ...
    <AppContent class="app-content">
      <ValiForm :form="form" :vali="vali">
        ...
        <FormInput
          ...
          :iconSrc="iconSrc"
          :disabled="true"
          v-model="introduce.cusMemberReference"
          @iconClick="chooseCustomer"
          v-if="form.classification == '1'"
        ></FormInput>
        <FormInput
          v-model="introduce.salesStage"
          type="tel"
          disabled="disabled"
          label="介绍环节"
          v-if="form.classification == '1'"
            ...
            <div class="cus" v-for="(cus, index) in concList" :key="index">
              <mark>
                ...
                <div class="accountCode">
                  {{ cus.provinceName }}{{ cus.cityName }}{{ cus.countyName
                  }}{{ cus.streetNum }}
                </div>
        ...
        <div>
          ...
          <aside class="edit">
            <div
              ...
              :key="index"
            >
              <div class="accountCode">{{ bank.bankName }}</div>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        infoId: "",
        salesStage: "", //介绍环节
        cusMemberReferenceId: "" //介绍人infoid
      },
  ...
  created() {
    ...
    if (this.$route.params.result) {
      if (this.$route.params.result.name) {
        this.form.bankInfoList = this.bankInfoList;
        this.form.concList = this.concList;
        //客户联系信息特殊处理
        for (var i = 0; i < this.concList.length; i++) {
          let array = this.concList[i].attrValue.split(",");
          this.concList[i].address = array[3];
          this.concList[i].zipCode = array[4];
          this.concList[i].mobile = array[5];
          this.concList[i].email = array[6];
          this.concList[i].guMobile = array[7];
        }
        this.form = JSON.parse(JSON.stringify(this.$route.params.result));
        this.form.source = "PAT06";
        //投被保人职业
        this.job.occupationName = this.$route.params.result.professionName;
        this.job.occupationCode = this.$route.params.result.profession;
        this.job.occupationLevel = this.$route.params.result.professionLevel;
        //生日
  ...
  methods: {
    ...
    addBank() {
      ...
      //新增银行卡用参数
      let params = {
        name: this.form.name, //客户姓名
        mobile: this.form.mobilePhone, //手机号
        certificateType: this.form.certificateType, //证件类型
        certificateNo: this.form.certificateNo //证件号码
      };
      this.goWithParams("bankMes", params);
    },
    //打开银行账户信息页面
    openBank(index) {
      this.$store.commit("plan/setState", {
      ...
      utils.ui.confirm({
        ...
        btnList: [
          ...
          },
          {
            msg: "确定",
            cb: () => {
              if (this.bankInfoList[index].id) {
                const params = {
                  cusMemberBankinfo: {
                    id: this.bankInfoList[index].id
                  }
                };
      ...
      utils.ui.confirm({
        ...
        btnList: [
          ...
          },
          {
            msg: "确定",
            cb: () => {
              if (this.concList[index].attributeId) {
                const params = {
                  cusMemberAttribute: {
                    attributeId: this.concList[index].attributeId
                  }
                };
    ...
    saveCustomer() {
      //出生日期和证件有效期转换为日期格式
      this.form.professionName = this.job.occupationName;
      this.form.profession = this.job.occupationCode;
      this.form.professionLevel = this.job.occupationLevel;
      this.form.professionName = this.job.occupationName;
      this.form.profession = this.job.occupationCode;
      this.form.professionLevel = this.job.occupationLevel;
      this.form.cusMemberReference = this.introduce.cusMemberReference;
      this.form.referralstage = this.introduce.salesStage;
      this.form.cusMemberReferenceId = this.introduce.cusMemberReferenceId;
      ...
      if (this.concList.length > 0) {
        for (var j = 0; j < this.concList.length; j++) {
          ...
          if (!this.concList[j].attributeId) {
