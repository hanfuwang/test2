...
export default {
  ...
  watch: {
    selectMain: {
      handler(newval) {
        ...
        this.mainSetting.liabList = this.selectMain.liabList;
  ...
  methods: {
    updateAmount() {
      //爱健康2020版 第二责任和主险保额联动 主险保额30%
      if (this.selectMainCode == "ATCB03") {
    ...
    handleSelectMain(item) {
      ...
      if (this.additionalRiskList.length == 0) {
    ...
    //处理特殊主险配置项
    dealMainSetting(item) {
      //延喜b  男性年金开始领取年龄 为65开始 去除第一个
      if (item.productCode == "RPPB01" && this.insurant.sex == "0") {
        item.attList.forEach(item1 => {
          if (item1.attCode == "drawStartAge") {
            if (item1.attValueList.length > 4)
              item1.attValueList = item1.attValueList.slice(1);
            this.mainSetting.drawStartAgeList = item1.attValueList;
          }
        });
      } else if (item.productCode == "RPPB01" && this.insurant.sex == "1") {
        //延喜b  女性年金开始领取年龄 为60开始
        item.attList.forEach(item1 => {
          if (item1.attCode == "drawStartAge") {
            item1.attValue = item1.attValueList[1].code;
            this.mainSetting.drawStartAge = item1.attValueList[1].code;
          }
        });
      }
    },
          ...
          switch (element.attCode) {
            ...
            case "amount": //基本保险金额
              if (mainProposalProductItem.liabList.length == 0) {
                ...
                mainProposalProductItem.liabList.length > 0 &&
                mainProposalProductItem.productCode == "ATCB03"
              ) {
                this.mainSetting.isAmount = true;
                // this.mainSetting.amount = +item.baseAmount;
              }
