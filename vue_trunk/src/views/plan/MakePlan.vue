...
export default {
  ...
  methods: {
    ...
    async chooseApplicant() {
      const insurantName = this.insurant.name;
      const holderRelation = this.insurant.holderRelation;
      //选择了本人就不能再重新选投保人
      if (holderRelation == "00") return;
      if (insurantName == "") {
      ...
      if (
        ...
        //获取客户列表
        const data = await this.queryCusList().catch();
        //只有一个
        if (data && data.length === 1 && data[0].value.length === 1) {
          //客户id
          const infoId = data[0].value[0].infoId;
          //获取客户详情
          const detailCus = await this.queryCusDetail(infoId).catch();
          if (!detailCus) return;
          //给投保人赋值
          this.$store.commit("plan/setState", {
            attr: "applicant",
    ...
    queryCusList() {
      const params = {
        ...
        attrKey: this.insurant.holderRelation, //关系
        infoId2: "", //客户2主键id
        flag: "3", // 3.建议书 4.财务分析 5.客户管理 6.日程 7.健康管理
        name: "",
    ...
    calcPremiumAgain() {
      //信息完整就要试算
      if (
        this.mainProposalProductList.length != 0 &&
        this.applicant.name != "" &&
        this.applicant.birthdate != "" &&
        this.applicant.cdOccupation.occupationCode != "" &&
        this.insurant.holderRelation != "" &&
        this.insurant.name != "" &&
        this.insurant.birthdate != "" &&
    ...
    isMsgEmpty() {
      ...
      if (this.applicant.cdOccupation.occupationCode == "") {
